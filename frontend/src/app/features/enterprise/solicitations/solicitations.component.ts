...
@Component({
  selector: 'app-enterprise-solicitations',
  standalone: true,
  imports: [
  ...
  template: `
    <div class="page">
      <div class="page-header">
        <h1><mat-icon>mail</mat-icon> Sollicitations reçues</h1>
        <p>Missions que des clients souhaitent confier à votre entreprise</p>
      </div>
      ...
      <mat-tab-group class="tabs">
        ...
        <mat-tab label="Refusées ({{ closed.length }})">
      ...
      <ng-template #listTpl let-items="items">
        <div class="empty" *ngIf="!loading && !items.length">
          ...
          <p>Aucune sollicitation</p>
        </div>

        <div class="cards" *ngIf="items.length">
          <mat-card class="sol-card clickable" *ngFor="let s of items" [routerLink]="['/enterprise/solicitations', s.id]">
            <div class="sol-header">
              <div>
                <h3>{{ s.mission_title }}</h3>
                <p class="client" *ngIf="s.client">
                  <mat-icon>person</mat-icon> {{ s.client.first_name }} {{ s.client.last_name }}
                </p>
              </div>
  ...
  styles: [`
    ...
    .page-header h1 { display: flex; align-items: center; gap: 10px; margin: 0 0 6px; font-size: 26px; }
    ...
    .cards { display: flex; flex-direction: column; gap: 16px; padding: 16px 0 24px; }
    .sol-card { padding: 20px; border-radius: 14px; border: 1px solid #e2e8f0; }
    ...
    .sol-header { display: flex; justify-content: space-between; gap: 12px; margin-bottom: 12px; }
    .sol-header h3 { margin: 0 0 4px; font-size: 18px; }
    .client { display: flex; align-items: center; gap: 4px; margin: 0; font-size: 13px; color: #64748b; }
    .client mat-icon { font-size: 16px; width: 16px; height: 16px; }
    .status-chip { padding: 4px 10px; border-radius: 999px; font-size: 11px; font-weight: 600; }
    ...
    .meta { display: flex; flex-wrap: wrap; gap: 12px; font-size: 13px; color: #475569; margin-bottom: 10px; }
    .meta span { display: inline-flex; align-items: center; gap: 4px; }
    .meta mat-icon { font-size: 16px; width: 16px; height: 16px; color: #3CB371; }
    .message { font-size: 14px; color: #64748b; font-style: italic; margin: 0 0 16px; padding: 10px 12px; background: #f8fafc; border-radius: 8px; }
...
export class EnterpriseSolicitationsComponent implements OnInit {
  ...
  get pending() { return this.solicitations.filter(s => s.status === 'pending'); }
  get accepted() { return this.solicitations.filter(s => s.status === 'accepted'); }
  get closed() {
    return this.solicitations.filter(s => ['rejected', 'cancelled', 'expired'].includes(s.status));
  }

  load(): void {
    this.loading = true;
    this.missionService.getMySolicitations(undefined, 'enterprise').subscribe({
      next: (data) => { this.solicitations = data; this.loading = false; },
      error: () => { this.loading = false; this.snackBar.open('Erreur', 'Fermer', { duration: 4000 }); },
    });
  }

  formatBudget(s: MissionSolicitation): string {
    return `${formatXOF(Number(s.mission_budget))} ${s.mission_currency || 'XOF'}`;
  }
